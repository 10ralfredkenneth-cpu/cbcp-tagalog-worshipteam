...
function SongLibraryPage() {
  ...
  return <main className="container mx-auto px-4 sm:px-6 py-14 md:py-20">
    ...
    <section className="sticky top-[80px] z-30 bg-background/95 backdrop-blur-md py-5 border-b border-accent/10 mb-10" aria-label="Song library controls">
      ...
      <div className="flex gap-2 overflow-x-auto py-4 -mb-2">{languages.map((lang) => (counts[lang] ?? 0) > 0 && <Button key={lang} variant={languageFilter === lang ? 'secondary' : 'ghost'} className="rounded-none shrink-0 text-xs" onClick={() => setLanguageFilter(lang)}>{lang} <span className="ml-2 text-muted-foreground">{counts[lang] ?? 0}</span></Button>)}</div>
